...
#if !defined(CONFIG_MODEM_UBLOX_SARA_U2_MANUAL_MCCMNO)
...
static const struct mdm_control_pinconfig pinconfig[] = {
	/* MDM_POWER */
	PINCONFIG(DT_UBLOX_SARA_U2_0_MDM_POWER_GPIOS_CONTROLLER,
		  DT_UBLOX_SARA_U2_0_MDM_POWER_GPIOS_PIN),

	/* MDM_RESET */
	PINCONFIG(DT_UBLOX_SARA_U2_0_MDM_RESET_GPIOS_CONTROLLER,
		  DT_UBLOX_SARA_U2_0_MDM_RESET_GPIOS_PIN),

	/* MDM_VINT */
	PINCONFIG(DT_UBLOX_SARA_U2_0_MDM_VINT_GPIOS_CONTROLLER,
			  DT_UBLOX_SARA_U2_0_MDM_VINT_GPIOS_PIN),

#ifdef DT_UBLOX_SARA_U2_0_MDM_TTLBUFF_GPIOS_CONTROLLER
	/* MDM_TTLBUFF */
	PINCONFIG(DT_UBLOX_SARA_U2_0_MDM_TTLBUFF_GPIOS_CONTROLLER,
			  DT_UBLOX_SARA_U2_0_MDM_TTLBUFF_GPIOS_PIN),
#endif
};

#define MDM_UART_DEV_NAME		DT_UBLOX_SARA_U2_0_BUS_NAME

#define MDM_POWER_ENABLE		0
#define MDM_POWER_DISABLE		1
#define MDM_RESET_NOT_ASSERTED		1
#define MDM_RESET_ASSERTED		0
#define MDM_TTLBUFF_ENABLE	0
#define MDM_TTLBUFF_DISABLE	1
...
K_THREAD_STACK_DEFINE(modem_rx_stack,
		       CONFIG_MODEM_UBLOX_SARA_U2_RX_STACK_SIZE);
struct k_thread modem_rx_thread;

/* RX thread work queue */
K_THREAD_STACK_DEFINE(modem_workq_stack,
		      CONFIG_MODEM_UBLOX_SARA_U2_RX_WORKQ_STACK_SIZE);
...
struct modem_iface_ctx {
	...
	/* network data */
	char net_operator[MDM_OPERATOR_LENGTH];
...
{
	LOG_HEXDUMP_DBG(packet, length, "recv");
}
#else
#define hexdump(...)
#endif
...
static void on_cmd_atcmdinfo_ops(struct net_buf **buf, u16_t len)
{
	int i = 0, param_count = 0;
	size_t value_size;
	char value[MDM_OPERATOR_LENGTH];

	value_size = sizeof(value);
	while (*buf && len > 0 && param_count < 3) {
		i = 0;
	...
	if (param_count == 3 && i > 0) {
		char op[MDM_OPERATOR_LENGTH];
		if(value[0] == '"')
		{
			for(u8_t z = 1; z < MDM_OPERATOR_LENGTH; z++)
			{
				if(value[z] == '"')
				{
					op[z-1] = '\0';
...
#ifdef CONFIG_MODEM_UBLOX_SARA_U2_MANUAL_MCCMNO
#endif
		if(strcmp(op, CONFIG_MODEM_UBLOX_SARA_U2_MANUAL_MCCMNO) == 0)
		{
			strcpy(ictx.net_operator, op);
		}
		return;
	}
...
static void modem_rx(void)
{
	...
	static const struct cmd_handler handlers[] = {
		...
		CMD_HANDLER("+COPS: ", atcmdinfo_ops),
...
{
	u32_t powered_on_val = 0;
	u8_t power_on_max = 10;
	u8_t power_on_cnt = 0;

#ifdef DT_UBLOX_SARA_U2_0_MDM_TTLBUFF_GPIOS_CONTROLLER
	gpio_pin_configure(ictx.gpio_port_dev[MDM_TTLBUFF_EN],
					   pinconfig[MDM_TTLBUFF_EN].pin, GPIO_DIR_OUT);
	...
	while(!powered_on_val)
	{
		gpio_pin_read(ictx.gpio_port_dev[MDM_VINT],pinconfig[MDM_VINT].pin, &powered_on_val);
		power_on_cnt++;

		if(power_on_cnt > power_on_max)
		{
			return -ETIMEDOUT;
		}
		k_sleep(1000);
...
restart:
	...
	send_at_cmd(NULL, "AT+COPS?", MDM_CMD_TIMEOUT);

	if(strlen(ictx.net_operator) > 0)
	{
		if(strlen(CONFIG_MODEM_UBLOX_SARA_U2_MANUAL_MCCMNO) > 0
			&&
			strcmp(ictx.net_operator, CONFIG_MODEM_UBLOX_SARA_U2_MANUAL_MCCMNO) == 0
			)
		{
			/* already connected to operator, start querying rssi */
			goto query_rssi;
		}
	}
	...
	ret = send_at_cmd(NULL, "AT+CGDCONT=1,\"IP\",\""
				CONFIG_MODEM_UBLOX_SARA_U2_APN "\"",
			  MDM_CMD_TIMEOUT);
	if (ret < 0) {
	...
	/* wait for +CREG: 1 (home network) or +CREG: 5 (roaming) notification (20 seconds max) */
	counter = 0;
...
NET_DEVICE_OFFLOAD_INIT(modem_sara_u2, "MODEM_SARA_U2",
			modem_init, &ictx,
			NULL, CONFIG_MODEM_UBLOX_SARA_U2_INIT_PRIORITY, &api_funcs,
			MDM_MAX_DATA_LENGTH);
