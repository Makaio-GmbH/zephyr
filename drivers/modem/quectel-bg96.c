...
#define LOG_DOMAIN modem_quectel_bg96
#define LOG_LEVEL 4
			...
			const u8_t *data, int timeout)
{
	...
	if(ictx.power_state.uart_disabled)
	{
		...
		api_wake(device_get_binding("MODEM_QUECTEL_BG96"));
	}
...
restart:
	...
	while (counter++ < 50 && ret < 0) {
		...
		ret = send_at_cmd(NULL, "AT+QSCLK=1", MDM_CMD_TIMEOUT);
...
*/
	...
	/* TODO */
	if(strcmp(DT_INST_0_QUECTEL_BG96_BUS_NAME, "UART_0") == 0)
	{
...
#endif
	} else if(strcmp(DT_INST_0_QUECTEL_BG96_BUS_NAME, "UART_1") == 0) {
...
NET_DEVICE_OFFLOAD_INIT(modem_quectel_bg96, "MODEM_QUECTEL_BG96",
			modem_init, &ictx,
			NULL, 80, &api_funcs,
			MDM_MAX_DATA_LENGTH);
